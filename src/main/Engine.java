...
public class Engine {
    ...
    public void loadAssembly(String pathname) throws NumberFormatException, IOException {
        ...
        while ((line = br.readLine()) != null && line.length() > 0) {
            ...
            if (format.equals("R")) {
                ...
                register = Integer.parseInt(instructionStringSplit[2].substring(1));
                instructionInt = instructionInt | shiftBinary(register, 18);
                // if (Integer.parseInt(instructionStringSplit[3]) > 0)
    ...
    private static int shiftBinary(int value, int shiftAmount) {
        ...
        // int bitSize = Integer.toBinaryString(Integer.valueOf(value)).length();

        if (value < 0)
            value = (int) (Math.pow(2, 18) + value);
    ...
    private Instruction decode(int instructionFetch) {
        ...
        // I-type
                ...
                || opcode == 11) {
            if (Integer.toBinaryString(imm).startsWith("1")) {
            int firstZeroIndex = Integer.toBinaryString(imm).indexOf('0');
            int temp = 0;
            for (int k = 0; k < 18 - firstZeroIndex; k++) {
            temp += (int) (Math.pow(2, k) *
            Integer.parseInt(Integer.toBinaryString(imm).charAt(17 - k) + ""));
            }
            imm = -1 * temp;
